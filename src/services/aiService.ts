...
export const suggestSkills = async (resumeData: ResumeData): Promise<string> => {
    try {
        const token = localStorage.getItem('authToken');
        const experience = resumeData.experience.map(e => `- ${e.jobTitle} at ${e.company}:\n${e.description}`).join('\n');
        const response = await apiRequest('/ai/suggest-skills', {
            method: 'POST',
            body: JSON.stringify({ jobTitle: resumeData.personalDetails.jobTitle, experience }),
            token
        });
...
export const getGeneralResumeAnalysis = async (resumeData: ResumeData): Promise<{ score: number; feedback: string[]; }> => {
    try {
        ...
        return response.success ? response.data : { score: 0, feedback: ['Could not analyze resume.'] };
    } catch (error) {
...
export const getATSAnalysis = async (resumeData: ResumeData, jobDescription: string): Promise<ATSAnalysisResult> => {
    try {
        ...
        return response.success ? response.data : { matchScore: 0, missingKeywords: [], suggestions: ['Could not analyze resume.'] };
    } catch (error) {
...
**Tone:** Be encouraging, professional, and friendly. Use markdown formatting for readability.`;
        ...
        return response.success ? response.data : "I'm having trouble connecting right now.";
    } catch (error: any) {

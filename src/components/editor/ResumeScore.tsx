import React, { useState, useEffect } from "react";
import { useResume } from "@/hooks";
import { getGeneralResumeAnalysis, getATSAnalysis } from "@/services/aiService";
import { LoadingSpinner } from "@/components/common";
import { AIIcon } from "@/components/icons";
import { ATSAnalysisResult } from "@/types";

const ResumeScore: React.FC = () => {
  const { resumeData, activeResumeId } = useResume();
  const [score, setScore] = useState<number | null>(null);
  const [feedback, setFeedback] = useState<string[]>([]);
  const [atsResult, setAtsResult] = useState<ATSAnalysisResult | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [jobDescription, setJobDescription] = useState("");
  const [jdFocused, setJdFocused] = useState(false);

  useEffect(() => {
    setScore(null);
    setFeedback([]);
    setAtsResult(null);
  }, [resumeData]);

  const handleAnalyze = async () => {
    setIsLoading(true);
    setError(null);
    setScore(null);
    setFeedback([]);
    setAtsResult(null);

    try {
      if (jobDescription.trim()) {
        const result = await getATSAnalysis(resumeData, jobDescription);
        setAtsResult(result);
      } else {
        const result = await getGeneralResumeAnalysis(resumeData);
        setScore(result.score);
        setFeedback(result.feedback || []);
      }

      if (activeResumeId) {
        localStorage.setItem(`resumeAtsScanned:${activeResumeId}`, "true");
        window.dispatchEvent(
          new CustomEvent("resume-ats-analyzed", {
            detail: { resumeId: activeResumeId },
          }),
        );
      }
    } catch (err) {
      setError("Failed to analyze resume. Please try again.");
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  };

  /* Returns an inline color value rather than a Tailwind class,
       so it works against the dark editor background */
  const getScoreColor = (s: number): string => {
    if (s > 85) return "#4ade80";
    if (s > 70) return "#fde047";
    return "#f87171";
  };

  const hasAnalysisRun = score !== null || atsResult !== null;

  return (
    <div
      className="rounded-xl p-5 mb-6"
      style={{
        background: "rgba(255,255,255,0.05)",
        border: "1px solid rgba(255,255,255,0.09)",
      }}
    >
      {/* ── Header ── */}
      <div className="flex items-center gap-2 mb-4">
        <h2 className="text-xl font-bold" style={{ color: "#F0FDF4" }}>
          Resume Analysis
        </h2>
        <div className="group relative">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 cursor-help"
            style={{ color: "rgba(209,250,229,0.40)" }}
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              fillRule="evenodd"
              d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
              clipRule="evenodd"
            />
          </svg>
          <div className="absolute bottom-full mb-2 w-72 bg-gray-800 text-white text-xs rounded py-2 px-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 -translate-x-1/2 left-1/2">
            Get an instant score and feedback. For a tailored analysis, paste a
            job description to get a match score and keyword suggestions for
            Applicant Tracking Systems (ATS).
            <svg
              className="absolute text-gray-800 h-2 w-full left-0 top-full"
              x="0px"
              y="0px"
              viewBox="0 0 255 255"
            >
              <polygon
                className="fill-current"
                points="0,0 127.5,127.5 255,0"
              />
            </svg>
          </div>
        </div>
      </div>

      {/* ── JD input ── */}
      <div className="mb-4">
        <label
          htmlFor="job-description-analyzer"
          className="block text-sm font-medium mb-1"
          style={{ color: "rgba(209,250,229,0.55)" }}
        >
          Job Description (Optional)
        </label>
        <textarea
          id="job-description-analyzer"
          value={jobDescription}
          onChange={(e) => setJobDescription(e.target.value)}
          rows={4}
          placeholder="Paste a job description for an ATS-focused analysis..."
          onFocus={() => setJdFocused(true)}
          onBlur={() => setJdFocused(false)}
          style={{
            background: "rgba(255,255,255,0.06)",
            border: jdFocused
              ? "1px solid rgba(74,222,128,0.52)"
              : "1px solid rgba(255,255,255,0.12)",
            boxShadow: jdFocused ? "0 0 0 3px rgba(74,222,128,0.10)" : "none",
            color: "#F0FDF4",
            borderRadius: "0.5rem",
            padding: "0.5rem 0.75rem",
            outline: "none",
            resize: "vertical",
            width: "100%",
            fontSize: "0.875rem",
          }}
        />
      </div>

      {/* ── Loading ── */}
      {isLoading && (
        <div className="flex flex-col items-center justify-center p-8 text-center">
          <LoadingSpinner />
          <p
            className="mt-2 text-sm"
            style={{ color: "rgba(209,250,229,0.65)" }}
          >
            Analyzing your resume...
          </p>
          <p
            className="mt-1 text-xs"
            style={{ color: "rgba(209,250,229,0.42)" }}
          >
            This may take a moment.
          </p>
        </div>
      )}

      {/* ── Analyze button ── */}
      {!isLoading && (
        <button
          onClick={handleAnalyze}
          className="w-full flex items-center justify-center gap-2 btn-primary py-2 px-4 rounded-lg"
        >
          <AIIcon />
          <span>{hasAnalysisRun ? "Re-analyze" : "Analyze My Resume"}</span>
        </button>
      )}

      {/* ── Error ── */}
      {error && (
        <p className="mt-4 text-sm" style={{ color: "#f87171" }}>
          {error}
        </p>
      )}

      {/* ── Results ── */}
      {!isLoading && hasAnalysisRun && (
        <div
          className="mt-6 border-t pt-4"
          style={{ borderColor: "rgba(255,255,255,0.07)" }}
        >
          {/* ATS result */}
          {atsResult && (
            <div>
              <div className="text-center mb-4">
                <p
                  className="text-sm"
                  style={{ color: "rgba(209,250,229,0.65)" }}
                >
                  ATS Match Score
                </p>
                <p
                  className="text-6xl font-bold"
                  style={{ color: getScoreColor(atsResult.matchScore) }}
                >
                  {atsResult.matchScore}
                </p>
                <p
                  className="text-xs mt-1"
                  style={{ color: "rgba(209,250,229,0.42)" }}
                >
                  This score estimates how well your resume matches the job
                  description for an automated screening system (ATS).
                </p>
              </div>
              <div className="mb-4">
                <h3 className="font-semibold mb-2" style={{ color: "#F0FDF4" }}>
                  Missing Keywords
                </h3>
                <div className="flex flex-wrap gap-2">
                  {atsResult.missingKeywords.length > 0 ? (
                    atsResult.missingKeywords.map((keyword, index) => (
                      <span
                        key={index}
                        className="text-xs font-medium px-2.5 py-0.5 rounded-full"
                        style={{
                          background: "rgba(234,179,8,0.15)",
                          color: "#fde047",
                          border: "1px solid rgba(234,179,8,0.25)",
                        }}
                      >
                        {keyword}
                      </span>
                    ))
                  ) : (
                    <p
                      className="text-sm"
                      style={{ color: "rgba(209,250,229,0.45)" }}
                    >
                      No major keywords seem to be missing. Great job!
                    </p>
                  )}
                </div>
              </div>
              <div>
                <h3 className="font-semibold mb-2" style={{ color: "#F0FDF4" }}>
                  AI Suggestions
                </h3>
                <ul
                  className="list-disc pl-5 space-y-2 text-sm"
                  style={{ color: "rgba(209,250,229,0.65)" }}
                >
                  {atsResult.suggestions.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              </div>
            </div>
          )}

          {/* General analysis result */}
          {score !== null && (
            <div>
              <div className="text-center mb-4">
                <p
                  className="text-sm"
                  style={{ color: "rgba(209,250,229,0.65)" }}
                >
                  Overall Score
                </p>
                <p
                  className="text-6xl font-bold"
                  style={{ color: getScoreColor(score) }}
                >
                  {score}
                </p>
              </div>
              <div>
                <h3 className="font-semibold mb-2" style={{ color: "#F0FDF4" }}>
                  Suggestions for Improvement:
                </h3>
                <ul
                  className="list-disc pl-5 space-y-2 text-sm"
                  style={{ color: "rgba(209,250,229,0.65)" }}
                >
                  {feedback.map((item, index) => (
                    <li key={index}>{item}</li>
                  ))}
                </ul>
              </div>
            </div>
          )}
        </div>
      )}
    </div>
  );
};

export default ResumeScore;
